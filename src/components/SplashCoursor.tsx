...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const BACK_COLOR = isDarkMode
      ? { r: 0, g: 0, b: 0 } // black background
      : { r: 1, g: 1, b: 1 }; // white background

    const splashColor = isDarkMode
      ? { r: 1, g: 1, b: 1 } // white smoke
      : { r: 0.1, g: 0.1, b: 0.1 }; // blackish ink
    const canvas = canvasRef.current;
    if (!canvas) return;

    const pointers: Pointer[] = [{
  ...pointerPrototype(),
  color: splashColor // ✅ override the default pointer color
}];
